...
- **`受領中` は「FCが受領を始めたが在庫数はまだ入らない」期間を表す**（2026-09-15 追加）。
  `update_status_estimate` が `received > 0` になった日を `受領開始日(L)` へ書き、`ifs` がそれを拾う。
  **在庫数(R)の判定より下**に置いてあるので、受領が終われば自動で `在庫あり` に変わる。
  受領開始日は上書きしないので、いつから受領が始まったかは残る。
  **`自宅発送` より上にある**ため、自宅発送の行も受領が始まれば `受領中` になる。
...
| `update_status_estimate` | **状態非依存**（v0.11.0で状態ゲートを撤去）。`納品プラン` にshipment ID / inboundPlanID があり、状態が `在庫あり` / `在庫なし` **以外**の行。shipmentが `CLOSED` または受領率90%以上で受領とみなす | 在庫数, 受領日, **受領開始日**（`received > 0` で1回だけ） |
...
- **FBA在庫が行に収まらないときは Todoist へ出す**: 実FBA在庫が仕入管理のロット行に配分
  ...
  **受け皿は「在庫あり/在庫なし行への配分 ＋ 受領中の行の購入数」**（2026-09-15 変更）。
  以前は配分だけと比べていたため、**FCが受領を始めてから在庫数が入る（受領率90%）までの
  ずれが全部そのまま通知されていた**。2026-09-12 09:30 の通知は 13件 7,642個あったが、
  **11件は部分受領中というだけ**で、15:30 には勝手に消えていた。1日4回この誤報が出ていた。
  受け皿を数えるときは**状態ではなく「受領開始日が入っていて在庫数が空か」で絞る**。
  状態は数式なので、条件に使うと下記の自己ロックと同じ循環が起きる。
  **未割当が5個以下の行は通知しない**（`IGNORED_QUANTITY`。2026-09-16 追加）。
  売り切って受け皿の無くなったASINへ顧客返品が数個だけ再入庫すると出る差で、
  **原因は返品だと分かっているが受け皿に持たせていない**。数個では打つ手が無く、
  6個以上の本当に行が消えたケースを埋もれさせるほうが損だと判断した。
  返品由来かどうかを区別したくなったら、ここを閾値ではなく返品数の列で表すこと。
...
- **状態を増やしたら「売上/日」の数式も直す**（2026-09-15）。売上/日 の `マネジメント` シートは
  仕入管理の `IMPORTRANGE` なので、**状態名を直接書いた `sumifs` を並べた列**がそのまま影響を受ける。
  `受領中` を足したとき、AF列(在庫数) の **94行**に `sumifs(..., "受領中", ...)` を追記した。
  足し忘れると**その行の購入数が在庫数から丸ごと落ちる**（B0FR3CZRGP の 10,097個が消えかけた）。
  状態名を列挙している列: **AF(在庫数)** / Y(発送済み) / Z(梱包依頼済み) / AA(未発送) / AB(自宅発送) /
  AC(最終発送日) / AE(到着予定日)。Y〜AB は状態別の内訳なので `受領中` は足していない
  （内訳の合計は AF と一致しなくなる）。**列挙をやめて「在庫あり・在庫なし以外」を足す形にすれば
  状態が増えても壊れないが、未対応。**
- **売上/日 の AC列(最終発送日)・AE列(到着予定日) は全行 `#REF!`**（2026-09-15 時点。未修理）。
  `index(query(...), 2, 1)` が、発送済みの行が無いと参照エラーになる。今回の変更とは無関係の既存破損。
- **AB列(FBA在庫)・AC列は壊れている**（2026-09-15 時点。未修理）。AB は 5行目と 6行目で
  参照範囲が違ううえ **50行目以降には数式が無い**。AC は `#REF!` で全壊しており、しかも
  現在の `ifs` に存在しない `"納品中"` を参照している。これらに依存する AA・Y・Z も連鎖して壊れている。
  **シート上のこの集計値を判断の根拠に使わないこと。** 未割当の判定は Python 側で完結している。
