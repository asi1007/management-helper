...
## `--air` の別名は Chatwork の照合語から「空輸」を落とす (2026-09-18)

`--air` が「空輸」を足すのは**プラン別名だけ**。Chatwork の本文は `【ノーマル】`、
ファイル名は `2026-09-18_ノーマル.pdf` / `0918ノーマル指示書.xlsx` で、いずれも納品分類のまま。

そのため `revert-shipment --alias "09/18ノーマル空輸" --delete-chatwork` が
**1件も見つけられなかった**（`_strip_plan_name_suffix` で解消）。取り下げたつもりで
古い指示書が残り、検品担当が新旧2通を持つことになる。

**取り下げ件数は必ず数える。** 1回の送付は**本文・ラベルPDF・指示書xlsx の3件**。
3件に満たなければ照合語が合っていない。
